...
- **Tercera imagen del hero de la landing `/landing/pienso-luego-medito` apuntaba a un archivo inexistente**: `heroImage3` referenciaba `/images/home-mindfulness-01-tiny.webp` que no existe en `public/images/` (solo existe `home-mindfulness.webp`). La imagen se renderizaba como 404 silencioso. Corregido en `landing/pienso-luego-medito.astro` (fallback) y `pienso-luego-medito-landing.yaml` (datos del CMS) para apuntar a `/images/home-mindfulness.webp`.
